...
# run-phase5c-rig.sh
#
# Phase 5c EDIT-PATH stress rig: regenerate the scene, build the standalone,
...
APP_PATH="Builds/Phase5cEditStress.app"
RIG_OUTPUT_DIR="$HOME/Library/Application Support/DefaultCompany/Voxel Terraria 1 Byte BrickMap/Phase5cEditStress"

echo "== Regenerating the scene =="
"$UNITY_BIN" -batchmode -quit -nographics -projectPath "$(pwd)" \
  -executeMethod Phase5aSceneBuilder.GeneratePhase5c -logFile phase5c_scene.log || {
    echo "SCENE GEN FAILED"; tail -n 40 phase5c_scene.log; exit 1; }
...
"$UNITY_BIN" -batchmode -quit -projectPath "$(pwd)" \
  -executeMethod CommandLineBuild.BuildPhase5cStandalone -logFile phase5c_build.log
...
if grep -q "Shader error" phase5c_build.log; then
  echo "SHADER ERROR in the build (kernel(s) will be silently dropped at runtime):"
  grep -E "Shader error|Shader warning" phase5c_build.log
  exit 1
fi

if [ ! -d "$APP_PATH" ]; then
  echo "BUILD FAILED. Tail of phase5c_build.log:"; tail -n 60 phase5c_build.log; exit 1
fi

echo "== Running (blocks until the rig quits itself) =="
open -n -W "$APP_PATH" --args -phase5crig
...
echo
echo "=================== phase5c_report.txt ==================="
cat "${LATEST}phase5c_report.txt"
echo "==========================================================="
ls "$LATEST"
grep -q "RESULT: FAILED" "${LATEST}phase5c_report.txt" && { echo; echo "!! RIG FAILED"; exit 1; }
exit 0
