...
for i in $(seq 1 "$RUNS"); do
  ...
  echo; echo "########## SIEGE RUN $i/$RUNS  cap ${TILECAP}  (cooling ${COOL}s, ${FREE}GB free) ##########"
  sleep "$COOL"
  open -n -W "$APP" --args -cleardeltas -siegeseconds "$SECS" -tilecap "$TILECAP"
  ...
  grep -E "^  WHOLE RUN|    (PASS|FAIL|note)|^  live slots peak|^  tiles |^  placed |^  PEAK TILE" "${D}siege_report.txt" | sed 's/^/  /'
