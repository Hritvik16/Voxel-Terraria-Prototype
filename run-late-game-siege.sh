...
for i in $(seq 1 "$RUNS"); do
  ...
  echo; echo "########## SIEGE RUN $i/$RUNS  (cooling ${COOL}s, ${FREE}GB free) ##########"
  sleep "$COOL"
  open -n -W "$APP" --args -cleardeltas -siegeseconds "$SECS"
  ...
  grep -E "^  WHOLE RUN|    (PASS|FAIL)|^  live slots peak|^  tiles |^  placed " "${D}siege_report.txt" | sed 's/^/  /'
