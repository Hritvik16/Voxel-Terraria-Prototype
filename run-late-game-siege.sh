...
ORPHAN="${ORPHAN:-on}"
CEILING="${CEILING:-0}"
APPLYBUDGET="${APPLYBUDGET:-0}"
[ "$APPLYBUDGET" != "0" ] && ABFLAG="-applybudget $APPLYBUDGET" || ABFLAG=""
...
for i in $(seq 1 "$RUNS"); do
  ...
  sleep "$COOL"
  open -n -W "$APP" --args -cleardeltas -siegeseconds "$SECS" -tilecap "$TILECAP" $OFLAG -ceiling "$CEILING" -targetlive "$TARGETLIVE" $ABFLAG
  ...
  grep -E "^  WHOLE RUN|    (PASS|FAIL|note)|^  live slots peak|^  tiles |^  placed |^  PEAK TILE|^  ORPHANED TILES|^  op-list total|^  GPU |^  SLOT CEILING|^  apply budget|^  pump" "${D}siege_report.txt" | sed 's/^/  /'
