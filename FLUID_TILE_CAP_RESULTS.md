...
**Status: SHIPPED as of 2026-09-11.** §§0–10 are the tile-cap investigation;
**§11** is its shipped state; **§13** is the `MAX_ACTIVE_FLUID` ladder and
**§14** its recommendation; **§15 is the shipped state of that recommendation.**

Shipped: `FLUID_TILE_POOL_CAPACITY = 1024`, the orphaned-tile fix, and
**`MAX_ACTIVE_FLUID = 750,000`**. Tested and deliberately **not** shipped:
`MAX_ACTIVE_FLUID = 1,500,000` — see §14.
...
# 15. SHIPPED: `MAX_ACTIVE_FLUID = 750,000` (2026-09-11)

§14's recommendation was accepted and shipped. **1,500,000 remains tested and
NOT shipped** — its status in §14 is unchanged.

## 15.1 Every site, audited

| site | change |
|---|---|
| `EngineConfig.MAX_ACTIVE_FLUID` | **500,000 → 750,000**, with the basis recorded at the constant |
| `FluidChaosRig`, `FluidTileCapRig`, `LateGameSiegeRig` | `_slotCapacity = 500000` → `EngineConfig.MAX_ACTIVE_FLUID` |
| `Phase5aSceneBuilder` ×3 | `intValue = 500000` → the constant |
| 3 rig header comments | prose stating "500,000" / "500K" corrected |
| 3 scenes | serialized `_slotCapacity: 500000`, regenerated by the builders during the sweep |
| `ARCHITECTURE_v8.6.md` | row **annotated**, not overwritten — see 15.2 |

**Two traps avoided.** `BRICK_POOL_CAP` is *also* `500000` and is a different
constant — untouched. Most scene matches for "500000" are `fileID: 11500000`
noise; only three scenes carry a real site. Rigs setting `_slotCapacity` to
8,192 or 65,536 are deliberate scenario choices, not references to the ceiling,
and were left alone.

`run-fluid-scale.sh` iterates `EngineConfig.MAX_ACTIVE_FLUID` directly and
picked the new value up with no edit, reporting "a 750000-slot pool therefore
holds about 250000 live voxels in one region" — which is what a single source
of truth is supposed to look like.

## 15.2 The spec doc is annotated, not overwritten

`ARCHITECTURE_v8.6.md`'s column is the **"v1 starting value"**, so rewriting the
number in place would falsify what the spec originally set. The row now carries
a pointer and a note beneath the table recording, explicitly, that **this ship
was not made on the spec's own raise-condition**: that condition is a necessity
test ("Phase 5 shows near-player scope insufficient") which was *not*
demonstrated. What was demonstrated is affordability, plus a reachable clamp
whose overflow is the same artifact class the §7.2 tile pool produced.

## 15.3 Regression sweep at the new ceiling

| rig | baseline | now | |
|---|---|---|---|
| `run-acceptance-rig.sh` (**terrain-only**) | — | **53 / 0** | ✅ |
| `run-phase5a-rig.sh` | 5 scenarios, ledger balanced | same | ✅ |
| `run-phase5c-rig.sh` | 170 / 0 | 170 / 0 | ✅ |
| `run-phase5d-rig.sh` | 19 / 0 | 19 / 0 | ✅ |
| `run-fluid-activity.sh` | 19 / 0 | 19 / 0 | ✅ |
| `run-phase6-sandbox.sh` | 43 / 0 | 43 / 0 | ✅ |
| `run-fluid-scale.sh` | 4 / 0 | 4 / 0 | ✅ |
| `run-fluid-tiled.sh` | 19 / 0 | 19 / 0 | ✅ |
| `run-phase6-brushguard.sh` | 16 / 14 | 16 / 14 | ⚠️ pre-existing, **unchanged** |
| EditMode | 499 / 0 | 499 / 0 | ✅ |
| **late-game siege, shipped config** | — | **8 / 0** | ✅ |

Brushguard's failing assertion **set** was diffed against last session's:
**byte-identical**. Unchanged by the ceiling, as expected, and already logged in
CLAUDE.md.

The terrain-only baseline measured **frame total p50/p99 7.19 / 10.83** and
Gate C's §4.3 upload p99 at **0.596 ms** against the 1.0 ms budget, peak bytes
2.52 MB against the 3.145 MB cap. Recorded as measured — CLAUDE.md notes §4.3
is "variable enough that whole runs pass", so this is **not** claimed as an
improvement caused by this change.

## 15.4 The siege at the new ceiling

Cooled 200s run, no `-ceiling` override — the shipped constant:

```
GPU slot pool  20.0 MB (sized by the CEILING 750,000)
live slots peak         321,391 / 750,000
PEAK TILE DEMAND        651  (cap 1024)
FROZEN-FLUID CENSUS     0 of 2,302 sampled  ->  0.0%
no tile outlived its chunk   0 of 739 resident (470 retired during the run)
lava+obsidian conserved      12,735 -> 12,735  (0.00%)
WHOLE RUN  p50 8.30  p99 45.28  max 138.30 ms
PASS 8  FAIL 0
```

**The frozen-fluid census is 0.0% at the new ceiling**, p50 8.30 sits inside the
established 7.31–8.49 baseline band, and tile demand *fell* to 651 — the
headroom the ceiling buys is not being spent on tiles at ordinary play volume.
